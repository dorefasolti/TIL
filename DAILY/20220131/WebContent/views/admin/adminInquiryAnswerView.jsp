...
<div id="admin_content">
	...
	<form action="<%= contextPath %>/inquiryDelete.ad" method="post">
        <div id="admin_answer_3">
            <div class="d-flex">
                <div class="p-2" id="admin_answer_all">총 게시글 <%= listCount %>개</div>
            ...
            <table border="1">
                ...
                <tbody>
                	...
                	<%} else { %>
                		<%for(Answer a : list) {%>
                			<tr>
                				<td><input type="checkbox" class="batch" name="checkList" value="<%= a.getQno() %>"></td>
...
<%--                 				<input type="hidden" name="qcontent" id="qcontent" value="<%= a.getqContent() %>">
	    ...
	    <div class="paging-area" align="center">
	        <% if(currentPage != 1) {%>
	        	<button onclick="location.href='<%= contextPath %>/inquiryList.ad?currentPage=<%= currentPage -1 %>'">&lt;</button>
	    	<% } %>
	    
	    	<% for(int i = startPage; i <= endPage; i++) {%>
	    		<% if(i != currentPage) {%>
	    		
	    			<button class="btn-outline-light" onclick="location.href='<%= contextPath %>/inquiryList.ad?currentPage=<%= i %>'"><%= i %></button>
	    		<%} else { %>
